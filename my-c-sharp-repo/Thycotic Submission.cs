...
namespace CarPricer
{
    ...
    public class PriceDeterminator
    {
        public decimal DetermineCarPrice(Car car)
        {
            ...
            //  After 10 years, it's value cannot be reduced further by age. This is not 
            //  cumulative.
            finalPrice = finalPrice * (1 - (car.AgeInMonths.LimitInt(10 * 12) * (decimal)0.005));
            ...
            //  percent (0.2). Do not consider remaining miles. After 150,000 miles, it's 
            //  value cannot be reduced further by miles.
            finalPrice = finalPrice * (1 - (car.NumberOfMiles.LimitInt(150000).MultiplesOf(1000) * (decimal)0.002));


            //  3. PREVIOUS OWNER
            //  If the car has had more than 2 previous owners, reduce its value 
            //  by twenty-five (25) percent. 
            if (car.NumberOfPreviousOwners > 2)
            {
                finalPrice = finalPrice * (decimal).75;
            }

            // 4. COLLISION
            // For every reported collision the car has been in, remove two (2) percent of it's value up to five (5) collisions.
            finalPrice = finalPrice * (1 - (car.NumberOfCollisions.LimitInt(5) * (decimal).02));
            ...
            {
                finalPrice = finalPrice * (decimal)1.1;
            }
    ...
    public static class CarPricerExtensions
    {
        public static int LimitInt(this int actualValue, int theLimit)
        {
            return (Math.Min(actualValue, theLimit));
        }
        public static int MultiplesOf(this int actualValue, int multiple)
        {
            return actualValue / multiple;
        }
    }
